...
const assists=p=>Math.max(0,Number.parseInt(p?.assists,10)||0);
const phone=value=>{
...
export async function saveAdminForm(form){
  ...
  if(form.id==='player-admin-form'){
    ...
    if(!p){
      ...
      p={id:uid('player'),createdAt:new Date().toISOString(),passwordHash:await sha256(pass),goals:0,assists:0};
      d.players.push(p);
    }else if(pass){
    ...
    const playerAssists=Math.max(0,Number.parseInt(String(fd.get('assists')||'0'),10)||0);
    Object.assign(p,{name:String(fd.get('name')).trim(),email,status:String(fd.get('status')),position:String(fd.get('position')||'').trim(),number:String(fd.get('number')||'').trim(),whatsapp:String(fd.get('whatsapp')||'').trim(),goals:playerGoals,assists:playerAssists,photo,bio:String(fd.get('bio')||'').trim()});
  ...
  if(form.id==='team-form'){
    d.team={...d.team,name:String(fd.get('name')).trim(),neighborhood:String(fd.get('neighborhood')).trim(),city:String(fd.get('city')).trim(),trainingPlace:String(fd.get('trainingPlace')).trim(),description:String(fd.get('description')).trim()};
