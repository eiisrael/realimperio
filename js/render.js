...
const playerAssists=p=>Math.max(0,Number.parseInt(p?.assists,10)||0);
...
const playerPosition=p=>p?.position==='Outro'?'Reserva':p?.position||'';
...
function positionOptions(current=''){
  ...
  const options=['Goleiro','Zagueiro','Lateral','Volante','Meia','Atacante','Reserva'];
  const extra=normalized&&!options.includes(normalized)?`<option selected>${escapeHtml(normalized)}</option>`:'';
  return `<option value="">Selecione</option>${extra}${options.map(x=>`<option ${normalized===x?'selected':''}>${x}</option>`).join('')}`;
}
...
function playerCard(p){
  const photo=safeImageSrc(p.photo),goals=playerGoals(p),assists=playerAssists(p);
  return `<div class="player-card-wrap">${p.number?`<div class="player-number">${escapeHtml(p.number)}</div>`:''}<article class="card player-card">${photo?`<img class="player-photo" src="${escapeHtml(photo)}" alt="Foto de ${escapeHtml(p.name)}">`:`<div class="player-placeholder"><span>${escapeHtml(initials(p.name))}</span></div>`}<div class="player-body"><h3>${escapeHtml(p.name)}</h3><div class="player-meta-row"><span class="player-position">${escapeHtml(playerPosition(p)||'Jogador')}</span><span class="badge badge-training">Gols: ${goals}</span><span class="badge badge-training">Assistências: ${assists}</span></div><p class="player-bio">${escapeHtml(p.bio||'Descrição do jogador ainda não adicionada.')}</p></div></article></div>`;
}

function coachCard(){
  const c=state.data.coach||{},photo=safeImageSrc(c.photo||'');
  if(!c.name)return `<article class="card coach-public-card coach-public-empty"><div class="coach-public-avatar"><span>TEC</span></div><div><span class="kicker">Comissão técnica</span><h3>Técnico a definir</h3><p>O nome do responsável técnico será publicado pela administração.</p></div></article>`;
  return `<article class="card coach-public-card"><div class="coach-public-avatar">${photo?`<img src="${escapeHtml(photo)}" alt="Foto de ${escapeHtml(c.name)}">`:`<span>${escapeHtml(initials(c.name))}</span>`}</div><div class="coach-public-body"><span class="kicker">Comissão técnica</span><h3>${escapeHtml(c.name)}</h3><strong class="coach-role">Técnico</strong>${c.bio?`<p>${escapeHtml(c.bio)}</p>`:'<p>Responsável técnico do Real Império FC.</p>'}${c.whatsapp?`<span class="coach-contact">Contato: ${escapeHtml(c.whatsapp)}</span>`:''}</div></article>`;
}
...
export function renderAccount(){
  ...
  if(isAdmin()){
    if(s.role==='admin'){
      const c=state.data.coach||{},photo=safeImageSrc(c.photo||'');
      v.innerHTML=`<div class="page-head"><div><span class="kicker">Conta do Técnico</span><h1>Minha conta</h1><p>Esta conta administrativa representa o responsável técnico do Real Império FC.</p></div></div><article class="card account-card account-state">${photo?`<img class="account-admin-photo" src="${escapeHtml(photo)}" alt="Foto de ${escapeHtml(c.name||'Técnico')}">`:`<div class="account-avatar">TEC</div>`}<h2>${escapeHtml(c.name||'Técnico do Real Império FC')}</h2><p class="form-help">Técnico · Administrador Geral</p><div class="form-actions account-actions"><button class="btn btn-gold" data-nav="admin">Abrir painel</button><button class="btn btn-danger" data-action="logout">Sair</button></div></article>`;
      return;
    }
    v.innerHTML=`<div class="page-head"><div><span class="kicker">Sessão ativa</span><h1>Minha conta</h1></div></div><article class="card account-card account-state"><div class="account-avatar">ADM</div><h2>Administração Total</h2><p class="form-help">Sua sessão administrativa está ativa neste navegador.</p><div class="form-actions account-actions"><button class="btn btn-gold" data-nav="admin">Abrir painel</button><button class="btn btn-danger" data-action="logout">Sair</button></div></article>`;
    return;
  }

  const p=currentPlayer();
  if(!p){v.innerHTML=emptyState('Sessão de jogador não encontrada.');return}
  const photo=safeImageSrc(p.photo),goals=playerGoals(p),assists=playerAssists(p),lbl=p.status==='approved'?'Aprovado':p.status==='rejected'?'Negado':'Pendente',cls=p.status==='approved'?'badge-approved':p.status==='rejected'?'badge-rejected':'badge-pending';
